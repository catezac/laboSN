...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if (r < p){
      _particle(i).flip();
      _naccepted++; // conto quante volte faccio il flip
    }
  } else {           // M(RT)^2
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      // se voglio che la distribuzione iniiale delle velocità sia una delta devo modificare queste righe:
      /*vx(i) = _rnd.Gauss(0.,sqrt(_temp));
      vy(i) = _rnd.Gauss(0.,sqrt(_temp));
      vz(i) = _rnd.Gauss(0.,sqrt(_temp));*/
      double casual = _rnd.Rannyu(0,6);
      if (int(casual)==0)  vx(i) = 1.6*sqrt(_temp);
      if (int(casual)==1)  vx(i) = -1.6*sqrt(_temp);
      if (int(casual)==2)  vy(i) = 1.6*sqrt(_temp);
      if (int(casual)==3)  vy(i) = -1.6*sqrt(_temp);
      if (int(casual)==4)  vz(i) = 1.6*sqrt(_temp);
      if (int(casual)==5)  vz(i) = -1.6*sqrt(_temp);
      sumv(0) += vx(i);
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_chi){
    ...
    //chi /= double(_npart);
    _measurement(_index_chi) = _beta * chi;
  }
