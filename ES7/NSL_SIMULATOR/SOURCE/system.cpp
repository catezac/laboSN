...
void System :: averages(int blk){
  ...
  if(_measure_cv){
    _average(_index_cv) = _beta*_beta * (_average(_index_cv) - pow(_average(_index_tenergy),2));
  } 
  
  if(_measure_gofr){
    for(int k = 0; k < _n_bins; k++){
      double deltaV = (4/3)*M_PI*(pow((k+1)*_bin_size, 3)- pow(k*_bin_size,3));
      //cout << deltaV(k)<< endl;
      _average(_index_gofr + k) /= _rho * _npart * deltaV;
    }
  }
  ...
  if (_measure_gofr) {
    ...
    for (int i = 0; i < _n_bins; i++) {
      ...
      coutf << setw(12) << blk
            << setw(12) << _bin_size*i //+ (_bin_size)/2
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
