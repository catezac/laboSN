...
Random :: Random(){
   ...
   ifstream Primes("/home/catezac/laboSN/PRNG/Primes");
   if (Primes.is_open()){
      Primes >> p1 >> p2 ;
   } else cerr << "PROBLEM: Unable to open Primes" << endl;
   Primes.close();
   
   ifstream input("/home/catezac/laboSN/PRNG/seed.in");
   string property;
   if (input.is_open()){
      while ( !input.eof() ){
         input >> property;
...
/* double Random :: RW3D_discrete(vector <double> x, vector <double> y, vector <double> z, int k, int a) {
   double p = floor(Rannyu(0,6)); // passo
   if(p==1) {  x[k]+=a  };
   if(p==2) {  x[k]-=a  };
   if(p==3) {  y[k]+=a  };
   if(p==4) {  y[k]-=a  };
